...
public class StoreDao {
    ...
    public int checkStoreReviewDetail(int storeIdx){
        String checkStoreReviewQuery="select (exists(select Review.productsID from Review " +
                ...
                "    join Store on Store.ID=Products.ID WHERE Store.ID=?)) AS reviewCount";
        return this.jdbcTemplate.queryForObject(checkStoreReviewQuery,int.class,storeIdx);

    }
    public GetStoreReviewDetailRes getStoreReviewDetail(int storeIdx){
        ...
        return this.jdbcTemplate.queryForObject(getStoreReviewQuery,(rs,rowNum)->new GetStoreReviewDetailRes(
