...
//            }
    ...
    public BaseResponse<GetProductPaymentRes> getProductPayment(@RequestBody GetProductPaymentReq getProductPaymentReq){


        try{
            int userIdxByJwt=jwtService.getUserIdx();
            //jwt에서 idx추출한것과 userIdx가 같지 않으면
            if(getProductPaymentReq.getUserIdx()!=userIdxByJwt){
                return new BaseResponse<>(BaseResponseStatus.INVALID_JWT);
            }
            GetProductPaymentRes getProductPaymentRes=productProvider.getProductPayment(getProductPaymentReq);
