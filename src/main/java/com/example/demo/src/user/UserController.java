...
public class UserController {
    ...
    /**
     * 상점명 기입 API
     * [POST] /users/log-in/store-names
     */
    @ResponseBody
    @PostMapping("/log-in/store-names")
    public BaseResponse<PostLoginStoreNameRes> addStoreName(@RequestBody PostLoginStoreNameReq postLoginStoreNameReq) {
        try {
            if(!isRegexStoreName(postLoginStoreNameReq.getStoreName())){
                return new BaseResponse<>(POST_USERS_INVALID_STORE_NAME); // 2012 : 상점명 기본 표현식 예외
            }
            int flag = userProvider.checkStoreName(postLoginStoreNameReq.getStoreName());
            if(flag == 1){
                return new BaseResponse<>(POST_USERS_EXISTS_STORE_NAME); // 중복된 상점명 예외
            }
            return new BaseResponse<>(new PostLoginStoreNameRes(postLoginStoreNameReq.getStoreName()));
        } catch (BaseException exception) {
            return new BaseResponse<>(exception.getStatus());
        }
    }
